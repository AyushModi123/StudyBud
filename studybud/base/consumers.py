...
def check_user_in_participants(room, user):
    ...
    return not room.participants.filter(Q(id=user.id)).exists(), room.participants.count()
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        new_participant, participant_count = await check_user_in_participants(room, user)        
        if new_participant:
